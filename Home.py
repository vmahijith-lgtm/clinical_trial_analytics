...
from src.data_harmonization import DataHarmonization
...
def clear_all_memory() -> None:
    ...
    for key in list(st.session_state.keys()):
        if key not in ['processing_started', 'processing_complete']:
    ...
    try:
        ...
        for sheet_name in excel_file.sheet_names:
            ...
            try:
                ...
                # Store only summary in session state
...
def load_data_from_cache(cache_key: str) -> Optional[pd.DataFrame]:
    """Load a specific dataframe from disk cache"""
    df = disk_cache.load_dataframe(cache_key)
    if df is not None:
        memory_manager.force_cleanup()
    return df
...
# Header
st.title("🔬 Clinical Trial Analytics Platform")
st.markdown("### Memory-Optimized Processing - ALL Data, Minimal RAM")

st.markdown("""
This platform provides:
- **Complete data processing** - Processes ALL files without memory crashes
- **Memory-safe operations** - Disk-based caching, one file at a time
- **Real-time quality checks** - Automated quality monitoring
- **On-demand loading** - Data loads only when needed
""")

st.divider()

# Sidebar with Memory Monitor & Settings
with st.sidebar:
    st.header("📊 System Status")
    
    # Memory monitoring
    with st.expander("💾 Memory Monitor", expanded=True):
        display_memory_metrics()
        
        if st.button("🧹 Force Cleanup", use_container_width=True):
            before = memory_manager.get_memory_info()['rss_mb']
            memory_manager.deep_cleanup()
            clear_all_memory()
            after = memory_manager.get_memory_info()['rss_mb']
            st.success(f"Freed {before - after:.1f} MB")
        
        # Show memory advice
        advice = memory_manager.get_memory_advice()
        if advice and "Memory usage is normal" not in advice:
            st.info(advice)
    ...
    if low_memory_mode:
        ...
        st.info("Low memory mode enabled")
    else:
        memory_manager.enable_low_memory_mode(False)
    ...
    if st.session_state.processing_complete:
        ...
        st.metric("Files Processed", len(catalog))
    ...
    batch_size = st.selectbox(
        ...
        options=[1, 3, 5],
        index=0,
        format_func=lambda x: "Single file (safest)" if x == 1 else f"Batch of {x}",
    ...
    max_rows = st.selectbox(
        ...
        index=1,
        format_func=lambda x: f"{x:,}" if x < 1000000 else "Unlimited",
        help="Higher = more complete data, more RAM"
    )
...
with col1:
    ...
    if st.button("🚀 Load and Process All Data", type="primary", use_container_width=True):
        ...
        with progress_container:
            ...
            # Processing phase
            st.markdown("#### 1️⃣ Processing Files (One at a Time)")
            ...
            try:
                ...
                for progress, results, processed, total in process_all_files_memory_safe(
                    ...
                    status_text.text(f"Processing file {processed}/{total}...")
                    ...
                    with stats_container:
                        ...
                        with stat_col2:
                            st.metric("✅ Sheets", results['sheets_processed'])
                        with stat_col3:
                            st.metric("✅ Rows", format_number(results['total_rows'], 0))
                        with stat_col4:
                            success_rate = (processed / total * 100) if total > 0 else 0
                            st.metric("✅ Progress", f"{success_rate:.0f}%")
